...
&emsp;&emsp;左值和右值的根本区别是左值能够取地址，右值不能够取地址。在下面的代码中，`c`是左值，`6`是右值
...
> 左值引用和右值引用
>
> &emsp;&emsp;在C++11中新增了一种引用，即所谓的“右值引用(rvalue reference)”，在使用“引用”这一术语是，指的是“左值引用(lvalue reference)”
>
> ```C
> // 左值引用
> int ival = 1024;
> int &lval = ival;                // lval是ival的另一个名字
> ```
>
> 右值引用
>
> &emsp;&emsp;返回非引用类型的函数，连同算数，关系，位以及后置递增/递减运算符，都是右值。我们不能将一个左值引用绑定这类表达式上，我们可以将一个`const`左值引用或者一个右值引用绑定到这类表达式上。
>
> ~~~C
> const int &r3 = i * 24;
> const int &&r4 = i * 24;
...
1. `&ch`
   ...
   - 不合法的左值，`&ch`取得是变量`ch`的地址，但是这个值并不知道存在计算机内存中的那个地方，所以并不是一个合法的左值
2. `cp`
   - 作为右值，`cp`是指变量的值，因为在指针其实就是一个数值，在使用的过程中这个数值被指向对应的内存位置
   - 作为左值，就是`cp`所处的内存位置
3. `&cp`：通`&ch`
4. `*cp`
   - 作为右值，`*cp`指向空间的值
   - 作为左值，`*cp`指向空间
5. `*cp + 1`
   - 作为右值，*由于拥有更高的优先级，\*cp指向具体字符‘a’，‘a’+1=‘b’
   - 作为左值，非法
